                ...
                <form onSubmit={handleAdminLogin} className="space-y-6 py-6 animate-fade-in">
                  ...
                  <div className="space-y-3 font-bold text-xs select-none">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setPasswordAttempt("Blac&N{0}@er123)x");
                        setIsAdminAuthenticated(true);
                        sessionStorage.setItem('admin_authenticated', 'true');
                        setLoginError('');
                      }}
                      className="w-full text-center text-[#9A7D32] hover:text-[#80631F] font-sans font-black text-[10px] uppercase tracking-wider py-2.5 transition-colors cursor-pointer block border border-[#E8DFC2] bg-[#FCFAF2] rounded-xl hover:bg-[#FAF4E1] mt-1.5 shadow-xs"
                    >
                      💡 Guest Sandbox Bypass (Auto-unlock)
                    </button>
                  </div>
                </form>
              ) : (
